    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] relative">
            ...
            <div className="relative w-full h-[35vh] md:h-[45vh] flex flex-col items-center justify-center overflow-hidden">
                ...
                <button
                    onClick={() => navigate('/users')}
                    className="absolute top-4 left-4 z-20 flex items-center gap-2 text-white/70 hover:text-white transition-colors bg-black/30 backdrop-blur-sm rounded-full px-4 py-2 text-sm font-semibold"
                >
...
const UserProfileTab = ({ profile }: { profile: PublicUserProfile }) => {
    const animeCount = new Set([
        ...(profile.watchList || []).map((i: any) => i.id),
        ...(profile.continueWatching || []).map((i: any) => i.animeId),
    ]).size;
    const mangaCount = new Set((profile.readList || []).map((i: any) => i.id)).size;
    ...
    [...(profile.watchList || []), ...(profile.readList || [])].forEach((item: any) => {
...
const UserMangaOverview = ({ profile }: { profile: PublicUserProfile }) => {
    ...
    const totalManga = readList.length;
